kex=p256_ntru509
sig=p256_falcon512
FizzDir=/home/fzhang/MT/fizz-MT/_build/bin
CertDir=../Certs
LogDir=Result
...
for i in `seq 0 20`;do
    ...
    sudo ip netns exec ns-router tc qdisc add dev veth1-router root netem loss ${half_loss_rate}% delay ${half_delay_time}
    sudo ip netns exec ns-router tc qdisc add dev veth2-router root netem loss ${half_loss_rate}% delay ${half_delay_time}
